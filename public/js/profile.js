...
$('#edit_password_user').validate({
	rules: {
		direction_edit: { required: true, minlength: 5},
	},
	messages: {
		direction_edit: "Su contraseña debe tener minimo 5 caracteres.",
	},
	submitHandler: function(form){
		$('#button_password').addClass('loading');
		$.ajax({
			type: 'POST',
			url: 'config/profile',
			data: {
				...
				$('#button_password').removeClass('loading');
				$.uiAlert({textHead: 'Cambios guardados correctamente',text: '',bgcolor: '#19c3aa',textcolor: '#fff',position: 'top-right',icon: 'checkmark box',time: 3,}); 
